...
export const DetalhesVeiculoModal = ({ 
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      default:
        return "bg-gray-100 text-gray-800 dark:text-slate-100 border-gray-200 dark:border-slate-700";
    }
  };
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  <div>
                    <p className="text-sm text-gray-600 dark:text-slate-300">Marca:</p>
                    <p className="font-medium">{veiculo.marca}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600 dark:text-slate-300">Modelo:</p>
                    <p className="font-medium">{veiculo.modelo}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600 dark:text-slate-300">Ano:</p>
                    <p className="font-medium">{veiculo.ano}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600 dark:text-slate-300">Placa:</p>
                    <p className="font-medium">{veiculo.placa}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600 dark:text-slate-300">Cor:</p>
                ...
                {manutencoes.length > 0 ? (
                  <div className="space-y-3">
                    {manutencoes.map((manutencao) => (
                      <div key={manutencao.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-slate-900/40 rounded-lg">
                        <div>
                          <h4 className="font-medium">{manutencao.tipo}</h4>
                          <p className="text-sm text-gray-600 dark:text-slate-300">Sistema: {manutencao.sistema}</p>
                        </div>
                        <div className="flex items-center space-x-3">
                          <Badge className={getStatusColor(manutencao.status)}>
                            {manutencao.status}
                          </Badge>
                          <span className="text-sm text-gray-600 dark:text-slate-300">{manutencao.proximaEm}</span>
                          <Button
                            size="sm"
                            variant="outline"
                            className="text-green-600 border-green-300 hover:bg-green-50"
